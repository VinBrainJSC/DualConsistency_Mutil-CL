...
class RandomGenerator_IRCAD_concat_strongaug(object):
    def __init__(self, output_size):
        self.output_size = output_size
...
# Adapt to SSL-based concatenated IRCAD
class BaseDataSets_IRCAD_SSL_concat_kfold(Dataset):
    def __init__(self, base_dir=None, split='train',fold_num = None, num=None, transform=None):
        self._base_dir = base_dir
        self.sample_list = []
        self.split = split
        self.transform = transform
...
#             with open(self._base_dir + '/train_slice.txt', 'r') as f1:
#                 self.sample_list = f1.readlines()
            with open(self._base_dir + '/msd_ir_folds.json', 'r') as f1:
                self.sample_list = json.load(f1)
                self.sample_list = self.sample_list['fold_{}'.format(fold_num)]['train_fold_{}'.format(fold_num)]
#             self.sample_list = [item.replace('\n', '') for item in self.sample_list]
            
#             print(self.sample_list[:100])
        elif self.split == 'val':
#             with open(self._base_dir + '/val.txt', 'r') as f:
#                 self.sample_list = f.readlines()
#             self.sample_list = [item.replace('\n', '') for item in self.sample_list]
            with open(self._base_dir + '/msd_ir_folds.json', 'r') as f1:
                self.sample_list = json.load(f1)
                self.sample_list = self.sample_list['fold_{}'.format(fold_num)]['val_fold_{}'.format(fold_num)]
...
#                             "/training_slice_{}_concat_SSL_h5/{}.h5".format(organ, case), 'r')
            h5f = h5py.File(self._base_dir +
                            "/training_slice_{}_concat_h5/{}.h5".format(organ, case), 'r')
#             h5f = h5py.File(self._base_dir +
#                             "/training_slice_{}_concat_h5_M/{}.h5".format(organ, case), 'r')
        else:
            h5f = h5py.File(self._base_dir + "/val_{}_concat_h5/{}.h5".format(organ, case), 'r')
#             h5f = h5py.File(self._base_dir + "/val_{}_concat_h5_M/{}.h5".format(organ, case), 'r')
...
class BaseDataSets_UpdateLabel(Dataset):
    def __init__(self, data = None, split='train',fold_num = None, num=None, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        sample = {}
        organ ='ROI'
        image = self.sample_list[idx]['image'][:].numpy()
        label = self.sample_list[idx]['label_{}'.format(organ)][:].numpy()
        sample = {'image': image, 'label_{}'.format(organ): label}
        if self.split == "train":
            sample = self.transform(sample)
        sample["idx"] = idx
...
# Adapt to SSL-based concatenated IRCAD
class BaseDataSets_IRCAD_SSL_concat_kfold_update(Dataset):
    def __init__(self, base_dir=None,fold_num = None, num=None, transform=None):
        self._base_dir = base_dir
        self.sample_list = []
        self.transform = transform
    ...
    def __getitem__(self, idx):
        ...
        organ ='ROI'
        h5f = h5py.File(self._base_dir +
                        "/training_slice_{}_concat_h5_updated_fold_{}/{}.h5".format(organ,self.fold_num, case), 'r')
        image = h5f['image'][:]
        label = h5f['label_{}'.format(organ)][:]
        sample = {'image': image, 'label_{}'.format(organ): label}
