...
def ROI_preprocess(val_img_Dir, msk_baseDir, vessel_msk_baseDir, organ, mode):
...
#         vessel_msk2_path = os.path.join(vessel_msk2_baseDir, label_file_name)
        ...
        if os.path.exists(msk_path):
            ...
            sitk.WriteImage(img_itk, '../data/IRCAD/image_ROI_{}/image_{}.nii.gz'.format(mode, str(idx)))
            sitk.WriteImage(vessel_label_itk,
                            '../data/IRCAD/label_vessel_ROI/image_{}_gt.nii.gz'.format(str(idx)))
...
def ROI_crop_preprocess(val_img_Dir, msk_baseDir, vessel_msk1_baseDir, vessel_msk2_baseDir, organ, mode):
    ...
    for case in val_img_path:
        ...
        if os.path.exists(msk_path):
            ...
            vessel_label_itk = sitk.GetImageFromArray(vessel_mask.astype(np.float32))
            vessel_label_itk.SetSpacing(spacing)
            sitk.WriteImage(img_itk, '/home/xuzhe/Segment/SSL4MIS/data/IRCAD_c/image_ROI_{}_ori/image_{}.nii.gz'.format(mode, str(idx)))
            sitk.WriteImage(vessel_label_itk,
                            '/home/xuzhe/Segment/SSL4MIS/data/IRCAD_c/label_vessel_ROI/image_{}_gt.nii.gz'.format(str(idx)))
    print("Converted val IRCAD volumes to ROI")


def ROI_crop_preprocess_nosplit(val_img_Dir, msk_baseDir, vessel_msk_baseDir, organ):
    val_img_path = sorted(glob.glob(val_img_Dir))
    for case in val_img_path:
        print(case)
        img_itk = sitk.ReadImage(case)
...
#             vessel_mask = combine_vessel_mask(vessel_mask1, vessel_mask2)
            ...
            vessel_label_itk = sitk.GetImageFromArray(vessel_mask.astype(np.float32))
            vessel_label_itk.SetSpacing(spacing)
            sitk.WriteImage(img_itk, '/workspace/vinbrain/namnguyen/arterial_liver_segmentation/MTCL-main/code/data/IRCAD_c/image_ROI_ori/image_{}.nii.gz'.format(str(idx)))
            sitk.WriteImage(vessel_label_itk,
                            '/workspace/vinbrain/namnguyen/arterial_liver_segmentation/MTCL-main/code/data/IRCAD_c/label_vessel_ROI/image_{}_gt.nii.gz'.format(str(idx)))
    print("Converted val IRCAD volumes to ROI")
...
#     vessel_msk2_baseDir = '../../data/IRCAD/label_portalvein/'
    ...
    ROI_crop_preprocess_nosplit(all_img_Dir, ROI_baseDir, vessel_msk_baseDir, organ)
