    ...
    for case in train_img_path:
        ...
        if os.path.exists(msk_path):
            ...
            for slice_ind in range(image.shape[0]):
                ...
                f = h5py.File(
                    '../../data/IRCAD_c/training_slice_{}_concat_h5/{}_slice_{}.h5'.format(organ, item, slice_ind), 'w')
...
def test_volume_process(test_img_Dir, test_prob_Dir, msk_baseDir, organ):
    ...
    for case in test_img_path:
        ...
        if os.path.exists(msk_path):
            ...
            print('---------------')
            f = h5py.File(
                '../../data/IRCAD_c/val_{}_concat_h5/image_{}.h5'.format(organ, str(idx)), 'w')
    ...
    for case in train_img_path:
        ...
        if os.path.exists(msk_path):
            ...
            for slice_ind in range(image.shape[0]):
                # tutorial of h5py write file: https://blog.csdn.net/yudf2010/article/details/50353292
                f = h5py.File(
                    '../../data/IRCAD_c/training_slice_{}_concat_h5_M/{}_slice_{}.h5'.format(organ, item, slice_ind), 'w')
                img = np.expand_dims(image[slice_ind], axis=0)
                prob_ = np.expand_dims(prob[slice_ind], axis=0)
                concat_img = np.concatenate((img, prob_), axis=0)
                # print(concat_img.shape)
...
# test volume h5 generate
def test_volume_process_M(test_img_Dir, test_prob_Dir, msk_baseDir, organ):
    test_img_path = sorted(glob.glob(test_img_Dir))
    for case in test_img_path:
        print(case)
        img_itk = sitk.ReadImage(case)
        ...
        if os.path.exists(msk_path):
            ...
            print('---------------')
            f = h5py.File(
                '../../data/IRCAD_c/val_{}_concat_h5_M/image_{}.h5'.format(organ, str(idx)), 'w')
            concat_img = np.concatenate((image, prob), axis=0)
            print(concat_img.shape)
            f.create_dataset('image', data=concat_img, compression="gzip")
            f.create_dataset('label_{}'.format(organ), data=mask, compression="gzip")
...
if __name__=='__main__':
    ...
    train_img_Dir =  '../../data/IRCAD_c/image_ROI_ori/*.nii.gz'
    organ = 'ROI'
    
#     # Sata Enhanced folder
#     train_prob_Dir = '../data/IRCAD_NEW/image_ROI/'
#     # New ROI vessel
...
#     test_volume_process(train_img_Dir, train_prob_Dir, msk_baseDir, organ) 
    
    # Meijering Enhanced folder
    train_prob_Dir = '../../data/IRCAD_NEW_M/image_ROI/'
    # New ROI vessel
    msk_baseDir = '../../data/IRCAD_NEW_M/label_vessel_ROI/'
...
#     test_prob_Dir = '../../data/IRCAD_NEW/image_ROI_test/'
    ...
    test_volume_process_M(train_img_Dir, train_prob_Dir, msk_baseDir, organ) 
    #train concatenated volume for 3D version
